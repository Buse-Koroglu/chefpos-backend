...
public class GetOrdersQueryHandler : IRequestHandler<GetOrdersQuery, PagedResult<OrderResponseDto>>
    {
        ...
        public async Task<PagedResult<OrderResponseDto>> Handle(GetOrdersQuery request, CancellationToken cancellationToken)
        {
            ...
            var hasLocationAccess = requestingUser.HasRoleAtLocation(Role.CASHIER, request.LocationId) || requestingUser.HasRoleAtLocation(Role.WAITER, request.LocationId) || requestingUser.HasRoleAtLocation(Role.KITCHEN, request.LocationId);
            ...
            var isWaiterOnly = requestingUser.HasRole(Role.WAITER) && !requestingUser.HasRole(Role.CASHIER) && !requestingUser.HasRole(Role.KITCHEN);
